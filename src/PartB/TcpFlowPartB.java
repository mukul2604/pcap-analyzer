...
import static PartA.analysis_pcap_tcp.alpha;

/**
 * Created by mukul on 3/4/17.
 */

public class TcpFlowPartB {
    private List <TcpFlowPacketPartB> srcList = new ArrayList<>();
    private List <TcpFlowPacketPartB> destList = new ArrayList<>();
    private List <TcpFlowPacketPartB> ackList = new ArrayList<>();
    ...
    private ConcurrentHashMap<Long, TcpFlowPacketPartB> ackHash = new ConcurrentHashMap<>();
    private HashMap<Long, TcpFlowPacketPartB> dupAckHash = new HashMap<>();
   // private HashMap<Integer, Float> timeStampHash = new HashMap<>();

    protected int FastRetransmit = 0;
    protected int reTransmit = 0;

    public TcpFlowPartB(int src, int dest) {
        this.sourcePort = src;
    ...
    public void push(TcpFlowPacketPartB flowPacket) {
        int TRIPLE_DUP_ACK = 3;
        TcpFlowPacketPartB possibleDup = null;
        ...
        if (flowPacket.getSourcePort()== sourcePort &&
           ...
           // timeStampHash.put()
            ...
            if (dupAckHash.containsKey(flowPacket.getSeqNo())) {
                TcpFlowPacketPartB dupPacket = dupAckHash.get(flowPacket.getSeqNo());
            ...
            if (ackHash.containsKey(flowPacket.getAckNo())) {
                TcpFlowPacketPartB sentPacket = ackHash.get(flowPacket.getAckNo());
            ...
            if (dupAckHash.containsKey(flowPacket.getAckNo())) {
                ...
                if ( value < TRIPLE_DUP_ACK) {
                    TcpFlowPacketPartB acp = dupAckHash.get(flowPacket.getAckNo());
    ...
    private void printTransactions(int no) {
        int srcBase = 1;
        int destBase = 0;
        TcpFlowPacketPartB srcP = srcList.get(no+srcBase);
        TcpFlowPacketPartB destP = destList.get(no+destBase);
